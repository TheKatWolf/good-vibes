...
Live data from 5 sources: - Bright Data (LinkedIn + Indeed job scraping) - FRED API (Federal Reserve economic data) - Montgomery Open Data Portal (ArcGIS Hub) - Anthropic Claude API (AI synthesis) ---## Running Locally### Prerequisites- Node.js 18+ - npm ### Setup1. Clone the repo  git clone https://github.com/YOUR_USERNAME/montgomery-transparency-gap  cd montgomery-transparency-gap 2. Install dependencies  npm install 3. Create your .env file (see API Keys section below)  cp .env.example .env 4. Start the backend  node server.js 5. Start the frontend (new terminal tab)  npm run dev 6. Open browser  http://localhost:5173 ---## API Keys — How to Get YoursYou will need 4 API keys. Create a `.env` file in the root of the project and add each one: ### 1. Bright Data (Job Scraping — LinkedIn + Indeed)This is the core data source. Required to pull live job postings. 1. Go to https://brightdata.com and create a free account 2. Navigate to Proxies & Scraping → Web Scraper API 3. Create a new zone — name it anything (e.g. montgomeryjobs) 4. Copy your API key from the zone settings 5. You'll also need two Dataset IDs:    - LinkedIn Jobs: gd_lpfll7v5hcqtkxl6l    - Indeed Jobs: gd_l4dx9j9sscpvs7no2  (These are public Bright Data dataset IDs — no setup needed) Add to .env: BRIGHTDATA_API_KEY=your_key_here ---### 2. FRED API (Federal Reserve Economic Data)Free, no credit card required. 1. Go to https://fred.stlouisfed.org/docs/api/api_key.html 2. Create a free account 3. Request an API key (instant approval) Add to .env: FRED_API_KEY=your_key_here ---### 3. Anthropic Claude API (AI Synthesis)Powers the live synthesis block. Requires credits (~$5 is plenty). 1. Go to https://console.anthropic.com 2. Create an account 3. Navigate to API Keys → Create Key 4. Add billing credits (minimum $5) at Plans & Billing Add to .env: ANTHROPIC_API_KEY=your_key_here ---### 4. Montgomery Open DataNo API key required. Data is pulled directly from Montgomery's public ArcGIS Hub: https://data.montgomeryal.gov ---## .env File FormatCreate a file called `.env` in the root directory: BRIGHTDATA_API_KEY=your_brightdata_key FRED_API_KEY=your_fred_key ANTHROPIC_API_KEY=your_anthropic_key Never commit this file. It is listed in .gitignore. ---## Tech Stack- Frontend: React + Vite - Backend: Node.js + Express - Charts: Recharts - Job Data: Bright Data (LinkedIn + Indeed scrapers) - Economic Data: FRED API - Civic Data: Montgomery Open Data Portal (ArcGIS) - AI Layer: Anthropic Claude API (Haiku) - Hosting: Vercel ---## Data Sources & Methodology**Job Postings:** Scraped daily from LinkedIn and Indeed for Montgomery, AL using 10 keyword searches covering data center, cybersecurity, network engineering, and adjacent tech roles. Deduplicated by normalized title and company name. **Salary Data:** Pulled directly from job posting salary ranges where disclosed. Average calculated across all postings with salary data. **Construction Data:** Live from Montgomery's ArcGIS Hub, fiscal year Oct–Sep. **Business Licenses:** Live from Montgomery Open Data Portal. **Unemployment:** FRED BLS data, Montgomery Metro MSA. ---## TeamBuilt by Good Vibes - Kat Wolfe & Jennifer Watters, Built for GenAI Works — World Wide Vibes Hackathon, March 2026Bright DataBright Data - All in One Platform for Proxies and Web ScrapingAward winning proxy networks, powerful web scrapers, and ready-to-use datasets for download. Welcome to the world's #1 web data platform.
